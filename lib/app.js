...
function installExtraModule() {
    const extraDevDependancyModule = [
        'node-sass',
        'sass-loader'
    ];

    const extraDependancyModule = [
        'react-router',
        'include-media',
        'classnames',
        'axios',
        'immutable',
        'redux', 'react-redux', 'redux-actions', 'redux-logger', 'redux-promise-middleware'
    ];

    spawnSync('npm', ['install', '--save-dev'].concat(extraDevDependancyModule), { stdio: 'inherit', shell: true });
    spawnSync('npm', ['install', '--save'].concat(extraDependancyModule), { stdio: 'inherit', shell: true });
}
